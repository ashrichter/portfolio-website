...
export default function HomePage(props:{
    ...
    const [bodyJSX, setBodyJSX] = useState<string>("home"); // Store current section name

    const isKeyExist = (keyStr: string) => { // Does sections object contain key?
        return props.sections.hasOwnProperty(keyStr);
    }
    
    const capitalizedKeys = Object.keys(props.sections).map(key => {
        return key.charAt(0).toUpperCase() + key.slice(1); // Capitalise the first letter for each section
    });
